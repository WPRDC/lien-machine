...
#   codes, two of which are different codes for the same PIN. (Upserts into 
#   the raw-liens table are covered by including the block lot in the unique 
#   keys. The active file is still problematic.)
...
# [X] Implement pushing this stuff to CKAN. (At present, this is just done
# through the separate pipe_*_from_csv.py scripts, which use CSV files 
# generated by yet another script by exporting data from the database
# made by this script (process_liens.py).)
