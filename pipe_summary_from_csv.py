...
if __name__ == "__main__":
    ...
    #kwparams = dict(resource_id='d1e80180-5b2e-4dab-8ec3-be621628649e', schema=schema, key_fields=key_fields, server='production', pipe_name='liens_summary_pipeline', fields_to_publish=fields_to_publish, clear_first=True)
    kwparams = dict(resource_name='Summary of liens to present (alpha)', schema=schema, key_fields=key_fields, server='test-production', pipe_name='liens_summary_pipeline', fields_to_publish=fields_to_publish, clear_first=True)
    if len(sys.argv) > 1:
